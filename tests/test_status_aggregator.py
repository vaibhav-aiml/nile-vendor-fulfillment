...
def test_status_partially_confirmed():
    reqs = [
        _mock_request(FulfillmentStatus.CONFIRMED),
        _mock_request(FulfillmentStatus.OUTREACH_IN_PROGRESS),
    ]
    assert compute_overall_fulfillment_status(reqs) == OverallFulfillmentStatus.PARTIALLY_CONFIRMED


def test_status_blocked_alternate_needed():
    # If any item is rejected or alternate needed, it must block the overall status
    reqs = [
        _mock_request(FulfillmentStatus.CONFIRMED),
        _mock_request(FulfillmentStatus.REJECTED),
        _mock_request(FulfillmentStatus.OUTREACH_IN_PROGRESS),
    ]
    assert compute_overall_fulfillment_status(reqs) == OverallFulfillmentStatus.BLOCKED_ALTERNATE_NEEDED

    reqs_alt = [
        _mock_request(FulfillmentStatus.CONFIRMED),
        _mock_request(FulfillmentStatus.ALTERNATE_NEEDED),
    ]
    assert compute_overall_fulfillment_status(reqs_alt) == OverallFulfillmentStatus.BLOCKED_ALTERNATE_NEEDED
...
def test_build_itinerary_status_response_counts():
    ...
    assert resp.overall_fulfillment_status == OverallFulfillmentStatus.BLOCKED_ALTERNATE_NEEDED
